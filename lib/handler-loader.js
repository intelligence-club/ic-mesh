...
async function executeFromSpec(spec, job, options = {}) {
  ...
  const tmpDir = path.join(os.tmpdir(), `ic-handler-${Date.now()}`);
  ...
  if (job.payload?.url) {
    ...
    execSync(`curl -sL --max-filesize ${(spec.resources?.max_input_mb || 500) * 1048576} -o "${inputFile}" "${parsedUrl.href}"`, { timeout: 120000 });
  }
  ...
  try {
    ...
    if (invoke.output === 'file') {
      ...
      for (const f of candidates) {
        try { content = fs.readFileSync(f, 'utf8'); break; } catch {}
      }
      if (content !== null) {
