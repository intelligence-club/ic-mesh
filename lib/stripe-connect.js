...
// Safe logging wrapper to prevent logger failures from crashing payment flows
function safeLog(eventType, data) {
  try {
    logger.stripeEvent(eventType, data);
  } catch (loggerError) {
    console.error(`STRIPE_${eventType}:`, data);
    console.error('Logger failure:', loggerError.message);
  }
}
...
if (!STRIPE_API_KEY) {
  console.warn('⚠ STRIPE_API_KEY not set — Stripe features disabled'); module.exports = {}; return;
}
...
async function createConnectedAccount(nodeId, email, country = 'US') {
  ...
  try {
    safeLog('create connected account', { nodeId, email, country });
    ...
    safeLog('connected account created', { nodeId, accountId: account.id });
    ...
    safeLog('account creation failed', { nodeId, error: error.message });
...
async function transferToNode(stripeAccountId, amountInts, nodeId) {
  ...
  try {
    ...
    safeLog('transfer failed', { nodeId, error: error.message, code: error.code });
