...
function stripeRequest(method, path, params, retryCount = 0) {
  return new Promise((resolve, reject) => {
      ...
      res.on('end', () => {
        try {
          ...
          if (parsed.error) {
            ...
            console.error(`⚠ Stripe API Error: ${method} ${path} - ${error.message} (${res.statusCode})`);
            reject(error);
          } else {
            resolve(parsed);
          }
        } catch (e) {
          console.error(`⚠ Stripe response parse error: ${method} ${path} - ${e.message}`);
    ...
    req.on('error', async (err) => {
      console.error(`⚠ Stripe request error: ${method} ${path} - ${err.message}`);
      ...
      if (retryCount < MAX_RETRIES && (err.code === 'ENOTFOUND' || err.code === 'ECONNRESET' || err.code === 'ETIMEDOUT')) {
        ...
        console.log(`🔄 Retrying Stripe request in ${backoffMs}ms (attempt ${retryCount + 1}/${MAX_RETRIES})`);
...
async function createConnectedAccount(nodeId, email, country = 'US') {
  ...
  try {
    console.log(`📋 Creating Stripe Connect account for node ${nodeId} (${email})`);
    ...
    console.error(`❌ Failed to create Stripe Connect account for ${nodeId}: ${error.message}`);
...
async function transferToNode(stripeAccountId, amountInts, nodeId) {
  ...
  try {
    console.log(`💰 Initiating transfer: ${amountInts} ints ($${amountUsd.toFixed(2)}) to ${stripeAccountId}`);
    ...
    console.log(`✅ Transfer initiated: ${transfer.id} (status: ${transfer.status || 'pending'})`);
    ...
    console.error(`❌ Transfer failed for ${nodeId}: ${error.message}`);
