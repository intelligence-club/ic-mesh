...
class Reputation {
  ...
  _calculateImpact(event) {
    switch (event.type) {
      ...
      case 'verification_passed': return +3.0;
      case 'verification_failed': return -10.0;
      case 'capability_confirmed': return +1.0;
      case 'capability_suspect': return -4.0;
      case 'submitter_rating': {
        const rating = event.details?.rating || 3;
        return (rating - 3) * 2; // 1=-4, 2=-2, 3=0, 4=+2, 5=+4
      }
  ...
  recalculate(nodeId) {
    ...
    for (const evt of events) {
      ...
      switch (evt.type) {
        ...
        case 'verification_failed': {
          const rating = evt.type === 'verification_passed' ? 5 :
                        evt.type === 'verification_failed' ? 1 :
                        (details.rating || 3);
          qualitySum += (rating / 5) * 100 * recencyWeight;
    ...
    const qualityScore = qualityCount > 0 ? qualitySum / qualityCount : DEFAULT_SCORE;
